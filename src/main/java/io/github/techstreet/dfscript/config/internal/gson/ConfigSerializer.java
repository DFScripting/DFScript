package io.github.techstreet.dfscript.config.internal.gson;
...
public class ConfigSerializer implements JsonSerializer<ConfigInstruction>, JsonDeserializer<ConfigInstruction> {
            ...
            for (String key : keys) {
                ...
                if (jsonElement.isJsonPrimitive()) {
                    ...
                    if (primitive.isString()) {
                        setting = io.github.techstreet.dfscript.DFScript.GSON.fromJson(primitive, StringSetting.class);
                    } else if (primitive.isBoolean()) {
                        setting = io.github.techstreet.dfscript.DFScript.GSON.fromJson(primitive, BooleanSetting.class);
                    } else if (primitive.isNumber()) {
                        Number number = primitive.getAsNumber();
                        if (number instanceof Integer) {
                            setting = DFScript.GSON.fromJson(primitive, IntegerSetting.class);
                        } else if (number instanceof Double) {
                            setting = io.github.techstreet.dfscript.DFScript.GSON.fromJson(primitive, DoubleSetting.class);
                        } else if (number instanceof Float) {
                            setting = io.github.techstreet.dfscript.DFScript.GSON.fromJson(primitive, FloatSetting.class);
                        } else if (number instanceof Long) {
                            setting = io.github.techstreet.dfscript.DFScript.GSON.fromJson(primitive, LongSetting.class);
                        }
                    }
                }
    ...
    public JsonElement serialize(ConfigInstruction obj, Type type, JsonSerializationContext context) {
        ...
        for (Map.Entry<String, ConfigSetting<?>> entry : obj.getSettingMap().entrySet()) {
            ...
            json.add(key, io.github.techstreet.dfscript.DFScript.GSON.toJsonTree(value));
        }
        return json;
    }
